...
pub struct ServerState {
    ...
    pub min_supported_client_version: Arc<String>,
    pub client_download_url: Arc<String>,
}
...
impl ServerState {
    pub async fn load_from_config(config: &BackendConfig) -> anyhow::Result<Self> {
        ...
        Self::load_with_security(StateInitialization {
            ...
            min_supported_client_version: config.server.min_supported_client_version.clone(),
            client_download_url: config.server.client_download_url.clone(),
            ledger_service,
            auth_service,
    ...
    pub fn load(data_dir: PathBuf) -> anyhow::Result<Self> {
        ...
        Self::load_with_security(StateInitialization {
            ...
            min_supported_client_version: config.server.min_supported_client_version,
            client_download_url: config.server.client_download_url,
            ledger_service,
            auth_service,
    ...
    fn load_with_security(initialization: StateInitialization) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            ngrok_warning_bypass_enabled,
            web_login_enabled,
            client_version: Arc::new(client_version),
            min_supported_client_version: Arc::new(min_supported_client_version),
            client_download_url: Arc::new(client_download_url),
            data_dir,
        })
    }
}
